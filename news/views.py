...
from followpost.models import FollowPostItem, Comments
...
def index(request):
  ...
  #  articles_recent = Article.objects.filter(published=True).order_by('-date_published')[:10]
...
def article(request, slug):
    ...
    comments = Comments.objects.filter(article_id = article_id).order_by('id')
