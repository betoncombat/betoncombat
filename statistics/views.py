...
def index(request):
    ...
    statistic_obj = Statistic.objects.filter().values('win','lost','fund','year','risk','profit')
    for val in statistic_obj:
        val['accuracy'] = int((float(val['win']) / ( float(val['win']) +  float(val['lost'])))*100)
        val['roi'] = int((float(val['profit']) /  float(val['risk']))*100)
    c = {'yearly_statistics':yearly_statistics,
         ...
         'content':content[0].content, 'statistic_obj':statistic_obj}
